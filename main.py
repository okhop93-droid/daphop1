...
async def start_bot(session_str, account_no):
    client = TelegramClient(
        ...
        connection_retries=None, retry_delay=15, auto_reconnect=True,
        device_model=f"DapHop_V{account_no}", system_version="14.0"
    )
    try:
        await client.start()
        print(f"✅ TK {account_no} - ONLINE!")
                ...
                for row in event.reply_markup.rows:
                    for button in row.buttons:
                        if "Đập Hộp" in button.text:
                            await asyncio.sleep(account_no * 0.8) # Delay né spam
                            try:
                                await event.click()
                                print(f"🚀 [TK {account_no}] Đã đập hộp!")
                            except: pass
        ...
        print(f"⚠️ TK {account_no} dừng do lỗi IP: {e}")

app = Flask('')
@app.route('/')
def home(): return "Bot đang trực chiến!"
...
async def main():
    ...
    for i, session in enumerate(SESSIONS):
        if len(session) > 50:
            ...
            await asyncio.sleep(30) # Chờ 30s giữa mỗi acc để né lỗi IP
