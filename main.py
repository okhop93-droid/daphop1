...
def db_get_user(uid):
    try:
        ...
        return res.data[0]
    except: return {"user_id": uid, "balance": 0}

# --- 🎯 PHẦN ĐẬP HỘP TỰ ĐỘNG (ĐÃ QUAY LẠI) ---
...
async def start(e):
    ...
    raise events.StopPropagation # Chặn spam 2 tin
...
async def otp_cmd(e):
    ...
    await e.reply(f"✅ Đã treo acc {data['p']}!")
...
def webhook():
    ...
    if match:
        ...
        asyncio.run_coroutine_threadsafe(bot.send_message(uid, f"💰 Nạp thành công +{amount:,}đ!"), loop)
    return jsonify({"status": "ok"}), 200

# --- KHỞI CHẠY ---
async def main():
    await bot.start(bot_token=BOT_TOKEN)
    # Tự động bật lại các clone cũ
