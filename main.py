...
app = Flask('')
@app.route('/')
def home(): return "SILENT_9ACC_ALIVE"
...
async def start_acc(session_str, acc_no):
    ...
    try:
        ...
        print(f"✅ Acc {acc_no} ({me.first_name}) đã ONLINE - Không spam thông báo.")

        @client.on(events.NewMessage(chats=BOT_GAME))
        async def work(e):
            # 1. Chỉ nhắn khi lấy được mã Code
            ...
            # 2. Chỉ nhắn khi Đập hộp thành công
            if e.reply_markup:
                for row in e.reply_markup.rows:
                    for btn in row.buttons:
                        if any(x in btn.text for x in ["Đập", "Hộp", "Mở"]):
                            ...
                            try:
                                await e.click()
                                await client.send_message(GR_LOG, f"💰 Acc {acc_no} ({me.first_name}) vừa húp quà!")
                            except: pass
