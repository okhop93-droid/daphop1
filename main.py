...
async def start_bot(session_str, account_no):
    # Mỗi acc giả lập 1 máy riêng để Telegram không quét IP
    client = TelegramClient(
        StringSession(session_str), API_ID, API_HASH,
        connection_retries=None, retry_delay=15, auto_reconnect=True,
        device_model=f"DapHop_Acc_{account_no}", system_version="14.0"
    )
                ...
                for row in event.reply_markup.rows:
                    for button in row.buttons:
                        if "Đập Hộp" in button.text:
                            await asyncio.sleep(account_no * 0.5) # Giãn cách bấm
                            try:
                                await event.click()
...
app = Flask('')
@app.route('/')
def home(): return "Đội hình 10 tài khoản trực chiến!"

def run_flask(): app.run(host='0.0.0.0', port=8080)

async def main():
    print("🚀 Đang khởi động 10 tài khoản...")
    for i, session in enumerate(SESSIONS):
        # Bật từng em cách nhau 15s để tránh lỗi Timestamp/IP
