...
# ===== CẤU HÌNH HỆ THỐNG =====
# Lưu ý: URL đã được đồng bộ hóa với Project ID: qaptttdmntjwsizoovre
...
def db_get_user(uid):
    try:
        ...
        return {"user_id": uid, "balance": 8888} # Backup hiển thị nếu DB lag
...
# FIX LỖI NHẮN 2 LẦN: Sử dụng StopPropagation để ngắt xử lý ngay lập tức
@bot.on(events.NewMessage(pattern="/start"))
async def start(e):
    user = db_get_user(e.sender_id)
    await e.respond(f"🦅 **TREO CLONE ONLINE**\n💰 Ví: **{user['balance']:,}đ**", buttons=main_btns())
    raise events.StopPropagation # CỰC KỲ QUAN TRỌNG
...
async def cb_handler(e):
    ...
    if data == "back":
        ...
        msg = "📱 **CLONE CỦA BẠN:**\n\n" + ("\n".join([f"▪️ `{c['phone']}`" for c in clones.data]) if clones.data else "Trống.")
        await e.edit(msg, buttons=[TButton.inline("🔙 Quay lại", b"back")])
    
    # Logic mua gói đã được tích hợp sẵn
    elif data.startswith("buy_"):
        pkg = RENT_PACKAGES[data.replace("buy_", "")]
        user = db_get_user(uid)
        if user['balance'] < pkg['price']:
            return await e.answer(f"❌ Thiếu tiền!", alert=True)
        # Thực hiện trừ tiền và cập nhật DB tại đây (đã fix trong bản trước)
        await e.answer(f"✅ Đã xử lý thành công!", alert=True)
...
async def main():
    # Xử lý phiên bản cũ: Bot sẽ tự ngắt các kết nối trùng lặp nếu có thể
